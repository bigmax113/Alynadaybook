...
- Загрузка файлов (PDF, DOCX, XLSX) в рамках пользовательской сессии
...
def _is_allowed(fname: str) -> bool:
    low = fname.lower()
    return low.endswith(".pdf") or low.endswith(".docx") or low.endswith(".xlsx") or low.endswith(".zip")
...
def _ingest_file_to_cache(session: Dict[str, Any], file_path: str, shown_name: str) -> None:
    text = ""
    if shown_name.lower().endswith(".pdf"):
        text = _extract_text_from_pdf(file_path)
    elif shown_name.lower().endswith(".docx"):
        text = _extract_text_from_docx(file_path)
    elif shown_name.lower().endswith(".xlsx"):
...
def create_or_get_session():
    """
    Фронт присылает session_id (из localStorage). Если нет — создаём.
    Возвращаем session_id и краткий статус.
    """
...
def upload_files():
    ...
    ZIP распаковывается, допустимы вложения.
    """
    ...
    for storage in uploaded:
        ...
        if str(dest).lower().endswith(".zip"):
            # распаковать zip, положить рядом
            try:
    ...
    # Пройтись по (включая распакованные zip'ом)
    for p in base.rglob("*"):
        if p.is_file() and _is_allowed(p.name) and not str(p).lower().endswith(".zip"):
            # уже внесён при прямой загрузке — пропустим
...
def export_xlsx():
    """
    Пишем одну книгу:
      A1: "Вопрос", A2: question
      A4: "Ответ", A5: многострочный ответ
    """
